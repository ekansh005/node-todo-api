...
describe('GET /todos', () => {
  it('should list all todos of user', (done) => {
    request(app)
    .get('/todos')
    .set('x-auth', aUsers[0].tokens[0].token)
    .expect(200)
...
describe('GET /todos/:id', () => {
  ...
  it('should NOT fetch todo of another user', (done) => {
...
describe('PATCH /todos/:id', () => {
  ...
  it('should clear completedAt when todo is not completed', (done) => {
    ...
    request(app)
    .patch(`/todos/${id}`)
    .set('x-auth', aUsers[1].tokens[0].token)
    .send({
      text: 'updated second',
    })
    .expect(200)
...
describe('POST /users/login', () => {
  it('should provide x-auth token for valid credentials', (done) => {
    ...
    .end((err, res) => {
      ...
      User.findById(aUsers[1]._id).then((oUser) => {
        expect(oUser.tokens[1]).toMatchObject({
          access: 'auth',
  ...
  it('should return 400 for invalid credentials', (done) => {
    ...
    .end((err, res) => {
      ...
      User.findById(aUsers[1]._id).then((oUser) => {
        expect(oUser.tokens.length).toBe(1);
        done();
      }).catch((e) => done(e));
    });
  });
